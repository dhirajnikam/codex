...
#[test]
fn validate_npm_integrity_rejects_non_sha512_integrity() {
    assert_eq!(
        validate_npm_integrity("sha256-not-a-sha512-digest"),
        Err("npm plugin source integrity must use sha512".to_string())
    );
}

#[test]
fn verify_npm_integrity_rejects_mismatched_archive() {
    let archive_bytes = npm_package_archive_bytes("@acme/plugin", "1.2.0");
    let different_archive_bytes = npm_package_archive_bytes("@acme/plugin", "1.2.1");

    assert_eq!(
        verify_npm_integrity(&archive_bytes, &npm_integrity(&different_archive_bytes)),
        Err("npm package archive did not match declared sha512 integrity".to_string())
    );
}
...
fn npm_package_archive_bytes(package: &str, version: &str) -> Vec<u8> {
...
fn npm_integrity(archive_bytes: &[u8]) -> String {
    format!(
        "sha512-{}",
        BASE64_STANDARD.encode(Sha512::digest(archive_bytes))
    )
}
